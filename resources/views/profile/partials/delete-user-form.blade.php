...
<section class="space-y-4">
  ...
  <div x-data="{ show: @js($errors->userDeletion->isNotEmpty()) }" x-cloak
    x-init="if (show) document.documentElement.classList.add('modal-open')" x-on:open-delete-modal.window="
      show = true;
    ...
    <template x-teleport="body">
      <div x-show="show" class="fixed inset-0 modal-zp" x-cloak>
        {{-- Backdrop --}}
        <div class="fixed inset-0 modal-z bg-black/40 backdrop-blur-sm" x-transition.opacity.duration.150ms
        ...
        <div class="absolute inset-0 grid place-items-center p-4 pointer-events-none">
          <form method="post" action="{{ route('profile.destroy') }}"
            ...
            x-ref="form" x-show="show" x-transition @submit.prevent="
            ...
            <div class="mt-4">
              <x-text-input x-ref="pwd" id="delete_password" name="password" type="password"
                class="block w-full input-dynamic" placeholder="{{ __('Enter your password to confirm') }}"
...
@once
  @push('styles')
    <style>
      ...
      .swal2-cancel.btn-secondary-ghost:hover {
