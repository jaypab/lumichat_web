        ...
        <form method="POST" action="{{ route('admin.appointments.assign', $appointment->id) }}" class="space-y-6" id="assignCounselorForm" novalidate>
          ...
          <div>
            ...
            <div class="relative mt-1">
              ...
              >
                {{-- ✅ placeholder to force an explicit choice --}}
                <option value="">Select an available counselor…</option>

                @php
                  // if controller tagged __blocked_same it also set available=0; we still show it under Busy with a reason
                  $available = collect($counselors)->filter(fn($x) => (int)($x->available ?? 0) === 1);
                  $busy      = collect($counselors)->reject(fn($x) => (int)($x->available ?? 0) === 1);
                @endphp
                ...
                {{-- ✅ show all non-selectable entries with reasons, incl. "Not allowed (requested change)" --}}
                @if($busy->isNotEmpty())
                  <optgroup label="Busy / Not selectable ({{ $busy->count() }})">
                    @foreach($busy as $c)
            ...
            <p id="counselorHelp" class="mt-2 text-[13px] text-slate-500">
              <span class="font-medium">Legend:</span> Available = selectable • Busy/Not selectable = disabled ·
  ...
  <script>
   document.addEventListener('DOMContentLoaded', () => {
    ...
    const setButtonState = () => {
      const hasChoice = Array.from(select.options).some(o => o.value && !o.disabled);
      btn.disabled = !select.value || !hasChoice;
    };
    setButtonState();
    ...
    form.addEventListener('submit', (e) => {
      if (!select.value) {
        ...
        Swal.fire({ icon:'warning', title:'Counselor required', text:'Please select a counselor before assigning.', confirmButtonColor:'#4f46e5' });
        return;
      }
      btn.disabled = true;
      btn.innerHTML = `
