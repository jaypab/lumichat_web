  ...
  <div class="max-w-5xl mx-auto p-6 space-y-6">
    ...
    <div class="flex items-center justify-between no-print fade-in">
      @if(($highRisk->id ?? null) && ($highRisk->text ?? null))
  <div class="no-print fade-in" style="--delay:.04s">
    <div class="mt-4 rounded-2xl border border-rose-200 bg-rose-50 p-4">
      <div class="flex items-start gap-3">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mt-0.5 text-rose-600" viewBox="0 0 24 24" fill="currentColor">
        ...
        <div class="flex-1">
          ...
          <div class="mt-1 text-xs text-rose-800/80">
            {{ \Carbon\Carbon::parse($highRisk->sent_at)->format('F d, Y • h:i A') }}
            <span class="ml-1 opacity-70">• Chat ID: #{{ $highRisk->id }}</span>
          </div>
          <blockquote class="mt-2 rounded-xl bg-white ring-1 ring-rose-100 p-3 text-sm text-slate-800">
            “{{ $highRisk->text }}”
          </blockquote>
          <div class="mt-2 flex items-center gap-2">
            <button type="button" onclick="copyHighRisk()"
              class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white text-slate-800 ring-1 ring-slate-200 hover:bg-slate-50">
              Copy line
            </button>
            <a href="{{ route('admin.chatbot-sessions.show', $session->id) }}#sessionPrintable"
               class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white text-slate-800 ring-1 ring-slate-200 hover:bg-slate-50">
              View in context
            </a>
          </div>
  ...
  <script>
    function copyHighRisk(){
      const text = @json($highRisk->text ?? '');
      if (!text) return;
      navigator.clipboard.writeText(text).then(()=>{
        if (window.Swal) Swal.fire({toast:true, position:'top-end', icon:'success', title:'Copied', timer:1500, showConfirmButton:false});
      });
    }
  </script>
@elseif($isHighRisk)
  {{-- High risk, but no specific line found (rare) --}}
  <div class="no-print fade-in" style="--delay:.04s">
    <div class="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-amber-900">
      Session is HIGH-risk, but the exact trigger message wasn’t located (message may be unreadable or absent).
    </div>
  </div>
@endif
