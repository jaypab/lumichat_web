...
@php
  ...
  // KPIs (still based on rows, but now conceptually "with/without presenting problem")
  $totalAppts     = $students->count();
  $uniqueStudents = $students->pluck('student_id')->filter()->unique()->count();

  // a row is considered "with presenting problem" if cn.presenting_problem is non-empty
  $withDx = $students->filter(function($row){
      return isset($row->presenting_problem) && trim((string)$row->presenting_problem) !== '';
  })->count();
...
<div class="max-w-6xl mx-auto space-y-6">
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div class="text-[11px] uppercase tracking-wide text-slate-500">With presenting problem</div>
      <div class="mt-1 text-2xl font-semibold text-emerald-700">{{ $withDx }}</div>
    </div>
    <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div class="flex items-center justify-between">
        <div>
          <div class="text-[11px] uppercase tracking-wide text-slate-500">No presenting problem</div>
        ...
        <label class="screen-only inline-flex items-center gap-2 text-xs text-slate-600">
          ...
          <span>Show only without presenting problem</span>
        </label>
      </div>
    </div>
  </div>
  ...
  <div id="print-counselor-show" class="space-y-5">
    ...
    <h1 class="hidden print:block text-xl font-semibold">
      Counselor Logs — {{ $cName }} ({{ $label }})
    </h1>

    {{-- Presenting Problem Summary (filterable chips) --}}
    @if(!empty($dxCounts))
      <div class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden relative" aria-labelledby="dx-summary-h">
        ...
        <div class="px-5 py-4">
          <div id="dx-summary-h" class="text-sm font-semibold text-slate-800 mb-2">
          ...
          <div role="list" class="flex flex-wrap gap-2">
            ...
            @foreach($dxCounts as $dxLabel => $dxCnt)
              {!! $pill((string)$dxLabel, (int)$dxCnt, false) !!}
            @endforeach
          </div>
        </div>
      </div>
        ...
        <table class="min-w-full text-sm leading-6">
          ...
          <thead class="bg-slate-50 text-slate-600">
            <tr class="text-left">
              ...
              <th class="px-5 py-3 font-medium">Presenting Problem</th>
            </tr>
          </thead>
          <tbody id="rows" class="divide-y divide-slate-100">
            @forelse($students as $row)
              ...
              <tr class="hover:bg-slate-50/60 {{ $noDxRow ? 'nodx' : '' }}"
                ...
                <td class="px-5 py-3 whitespace-nowrap">{{ $scheduled }}</td>
                <td class="px-5 py-3">{!! $badge($presenting) !!}</td>
              </tr>
            @empty
              <tr>
...
{{-- Client-side filtering (presenting problem chips, search, no-presenting toggle) --}}
