@extends('layouts.counselor') {{-- or layouts.app if you share the layout --}}
...
<div class="max-w-7xl mx-auto p-6 space-y-6">
    ...
    <div class="p-5 sm:p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      <div>
        ...
        <p class="text-white/80 text-sm mt-0.5">
          Your counselor updates and system alerts.
        </p>
      </div>
      ...
      <div class="flex items-center gap-2">
        ...
        <form method="POST" action="{{ route('counselor.notifications.mark_all') }}">
          @csrf
  ...
  @php
    // Group into date buckets; handle string timestamps safely
    $groups = [
        'Today'      => [],
        'Yesterday'  => [],
        'This Week'  => [],
    ...
    foreach ($notifications as $n) {
        $date = $n->created_at instanceof \Illuminate\Support\Carbon ? $n->created_at : Carbon::parse($n->created_at);
      ...
      @if(count($items) > 0)
        ...
        @foreach ($items as $n)
          @php
            ...
            $body  = $data['body'] ?? '';
            // Controller already sanitizes URL; we intentionally DO NOT navigate to avoid 404s
            $url   = $data['url'] ?? null;
            $created = $n->created_at instanceof \Illuminate\Support\Carbon ? $n->created_at : Carbon::parse($n->created_at);
          @endphp

          <div class="nb-item p-4 flex items-start gap-3 rounded-xl" data-row="{{ $n->id }}">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="text-xs text-gray-400 mt-1" title="{{ $created->toDayDateTimeString() }}">
                {{ $created->diffForHumans() }}
              </div>

              {{-- Optional: show a tiny hint if a (safe) link exists, but do not navigate --}}
              @if($url)
                <div class="text-[11px] text-gray-400 mt-1">Link provided by system</div>
              @endif
            </div>
          </div>
        @endforeach
...
<script>
  const NB_MARK_URL_TEMPLATE = @json(route('counselor.notifications.mark', ['id' => '__ID__']));
  ...
  (function(){
    ...
    document.addEventListener('click', async (e) => {
      ...
      try{
        ...
        // Decrement the header bell badge, if present
