    ...
    <aside id="sidebar" class="sidebar-shell">
      ...
      @php
        $mainLinks = [
          ...
          ['label' => 'Announcements', 'route' => 'student.announcements', 'icon' => 'megaphone.png'],
          ...
          <ul class="space-y-1">
            @foreach ($mainLinks as $item)
              @if ($item['label'] === 'Appointment')
                ...
                @if ($showAppointment)
                  <li>
                    <a href="{{ $apptRoute }}" @class(['nav-item', 'nav-item--active' => $apptIsActive, 'relative' => true])
                      id="nav-appointment-link" data-tip="{{ $apptLabel }}">
              ...
              <li>
                <a href="{{ $href }}" @class([
                ...
                ]) @if($extraId) id="{{ $extraId }}" @endif data-tip="{{ $item['label'] }}">
                  <img src="{{ asset('images/icons/' . $item['icon']) }}" alt=""
                    class="sidebar-icon icon-white opacity-55 group-hover:opacity-100 transition-opacity">
                  ...
                  @if($item['label'] === 'Announcements' && Auth::check())
                    @php
                      ...
                      $newCount = \App\Models\Announcement::active()
                        ->where('created_at', '>', $lastSeen)
                        ->count();
                    @endphp
                    @if($newCount > 0)
                      <span
                        class="ml-auto flex items-center justify-center min-w-[1.25rem] h-5 px-1.5 rounded-full bg-rose-500 text-[10px] font-bold text-white shadow-[0_2px_8px_rgba(244,63,94,0.4)] transition-all animate-in fade-in zoom-in duration-300">
        ...
        <div class="header-inner flex items-center justify-between overflow-visible">
          ...
          <div class="flex items-center gap-3">
            ...
            @if(request()->routeIs('chat.index'))
              <span
                class="hidden sm:inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-200">
                Live
              </span>
            @endif
          </div>
                  ...
                  <a href="{{ route('settings.index') }}" class="dropdown-item">
                    <span class="dropdown-item-icon" aria-hidden="true">
                      <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        ...
                        <path
                          d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 11-2.83 2.83l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 11-4 0v-.09a1.65 1.65 0 00-1-1.51 1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 11-2.83-2.83l.06-.06A1.65 1.65 0 005 15.4a1.65 1.65 0 00-1.51-1H3.4a2 2 0 110-4h.09A1.65 1.65 0 005 8.89a1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 112.83-2.83l.06.06A1.65 1.65 0 009.11 5c.2-.81.93-1.39 1.76-1.4h.26a2 2 0 114 0h.09c.83.01 1.56.59 1.76 1.4a1.65 1.65 0 001.51 1 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 112.83 2.83l-.06.06A1.65 1.65 0 0019 10.11c.81.2 1.39.93 1.4 1.76v.26a2 2 0 110 4h-.09c-.83.01-1.56.59-1.76 1.4z" />
                      </svg>
                    </span>
                    <span>Settings</span>
                  </a>
