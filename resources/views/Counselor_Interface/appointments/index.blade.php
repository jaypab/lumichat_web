  ...
  <form method="GET" action="{{ route('counselor.appointments.index') }}" class="mb-3">
    <div class="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
      <div class="md:col-span-3">
        ...
        <select name="status" class="w-full h-10 bg-white border border-slate-200 rounded-xl px-3 text-sm">
      ...
      <div class="md:col-span-3">
        ...
        <select name="period" class="w-full h-10 bg-white border border-slate-200 rounded-xl px-3 text-sm">
      ...
      <div class="md:col-span-4">
        ...
        <div class="relative">
          <input id="q" name="q" value="{{ $q }}" placeholder="Name or email"
                 class="w-full h-10 bg-white border border-slate-200 rounded-xl pl-10 pr-3 text-sm">
      ...
      <div class="md:col-span-2 flex gap-2">
        <a href="{{ route('counselor.appointments.index') }}"
           class="h-10 inline-flex items-center rounded-xl bg-white px-4 ring-1 ring-slate-200">Reset</a>
        <button class="h-10 inline-flex items-center rounded-xl bg-indigo-600 text-white px-5">Apply</button>
      </div>
    </div>
  </form>
      ...
      <table class="min-w-full text-sm leading-6">
        ...
        @forelse($appointments as $row)
          @php
            ...
            // unified chip styles (matches your other pages)
            $statusMap = [
            ...
            // pretty label
            $statusLabel = $row->status === 'no_show' ? 'No Show' : ucfirst($row->status);
          @endphp
          <tr class="align-middle even:bg-slate-50 hover:bg-slate-100/60 transition">
            ...
            <td class="px-6 py-4 text-slate-700">{{ $row->student_name }}</td>
            <td class="px-6 py-4">
            ...
            <td class="px-6 py-4">
              <span class="relative inline-flex items-center h-7 w-[112px] rounded-full text-xs font-medium leading-none {{ $cls }}">
                ...
                <span class="mx-auto">{{ $statusLabel }}</span>
              </span>
            </td>
        ...
        @empty
          <tr><td colspan="6" class="px-6 py-10 text-center text-slate-500">No appointments found.</td></tr>
        @endforelse
        </tbody>
      </table>
    </div>
...
<script>
  // small UX sugar: type → auto-submit after pause
  const q = document.getElementById('q'); const form = q?.closest('form');
  let t = null; q?.addEventListener('input', ()=>{ clearTimeout(t); t=setTimeout(()=>form?.submit(), 350); });
</script>
@endsection
